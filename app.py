...
with st.expander("👋 New here? Click to see how to use this Lab"):
    ...
    2. **📚 Ingredient Library (Pyrfume)**: Browse 20,000+ molecules and scientific odor ratings.
    3. **🔬 Molecular Lab**: See the chemical "skeleton" of ingredients using SMILES codes.
    4. **📂 My Archive**: Upload your personal JSON scent collections.
    """)
...
with tab1:
    ...
    col1, col2 = st.columns(2) # FIXED: Added '2' to solve your TypeError
    ...
    with col2:
        ...
        if st.button("Find Similar Market Fragrances", help="Calculates similarity based on your selected notes."):
            ...
            scores = sorted(list(enumerate(sig[user_idx])), key=lambda x: x[1], reverse=True)
            
            for i in range(1, 4):
                match = df.iloc[scores[i][0]]
                st.info(f"✨ **{match['name']}** ({match['family']})\nNotes: {match['notes']}")

# --- TAB 2: INGREDIENT LIBRARY (REBRANDED RESEARCH HUB) ---
with tab2:
    st.header("Global Ingredient Library")
    st.info("💡 **What is this?** This section pulls from professional scientific archives (Pyrfume) to show you how different molecules behave.")
    
    dataset_choice = st.selectbox("Select Study/Library", 
                                ["bushdid_2014", "keller_2016", "snitz_2013"],
                                help="Each study offers different data, from molecular weights to human intensity ratings.")
    ...
    if st.button("Search Library"):
        try:
            ...
            st.error(f"This specific file is unavailable in this study. Error: {e}")
...
with tab3:
    ...
    with col_a:
        molecule_name = st.text_input("Ingredient Name", "Vanillin", help="The common name for your ingredient.")
        smiles_input = st.text_input("SMILES String", "O=Cc1cc(OC)c(O)cc1", 
                                   help="A standard chemical code. RDKit uses this to 'draw' the molecule.")
